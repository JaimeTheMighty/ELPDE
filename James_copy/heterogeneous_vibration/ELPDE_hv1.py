# -*- coding: utf-8 -*-
"""
Created on Tue Aug 20 13:49:42 2019

@author: James Lau
"""


#This version attempts to solve a heterogeneous vibration equation
...
def get_M_int(ncoef, xyt, deg, terms):
    ...
    for l in range(len(xyt)):
        for k in range(deg + 1):
            for m in range(k+1):
                for n in range(ncoef[k][m]):
                    for nterm in range(len(terms)): ###each run in this loop is for a separate term in the LHS of the PDE. 
                        temp = coe(xyt[l][0],xyt[l][1],nterm+1) ##Retrieve the coefficient
                        ###terms has three components: (alpha, beta, gamma)
                        for alpha in range(0,terms[nterm][0]):
                            temp = temp*(n - alpha)
                        for beta in range(0,terms[nterm][1]):
                            temp = temp*(m - n - beta)
                        for gamma in range(0, terms[nterm][2]):
                            temp = temp*(k - m - gamma)
                        exp1 = max(0, n - terms[nterm][0]) 
                        ##avoid negative exponents for 0. 
                        #If negative exponent, the if statement below will set this to zero anyway
                        exp2 = max(0, m - n - terms[nterm][1])
                        exp3 = max(0, k - m - terms[nterm][2])
                        temp = temp*(xyt[l][0]**(exp1))*(xyt[l][1]**(exp2))*(xyt[l][2]**(exp3))
                        if terms[nterm][0] > n or terms[nterm][1] > m-n or terms[nterm][2]>k-m:
                            temp = 0.0
...
def LPGenELPDE(Disc, deg, terms, BC1, BC2, BC3, BC4, BC5, BC6,\
               NBC1, NBC2, NBC3, NBC4, NBC5, NBC6, l1_norm, norm_lim, norm_lim_int):   
...
##Returns the governing equation violation
def evalWithA(x, y, t, A, deg):
     ...
     for k in range(deg + 1):
          for m in range(k+1):
              for n in range(ncoef[k][m]):
                  partialt = A[k][m][n]*(x**n)*(y**(m - n))*(t**max(k - m-2, 0))*max((k-m)*(k-m-1),0)
                  partialx = A[k][m][n]*(x**max(n-2,0))*(y**(m - n))*(t**(k - m))*max(n*(n-1),0)
                  partialy = A[k][m][n]*(x**n)*(y**max(m - n-2,0))*(t**(k - m))*max((m-n)*(m-n-1),0)
                  retVal = retVal + partialt-coe(x,y,1)*partialx - coe(x,y,2)*partialy
     return retVal
...
#               XY.append([cx,cy])
     ...
     ####Plot for each t
     uArray=np.array([[0.0 for x in X] for y in Y])
     
     if shouldPlot == True:
         for t in plotT:
             for nx in range(len(X)):
                 for ny in range(len(Y)):
                     eqval=evalWithA(X[nx], Y[ny], t, A, deg)
                     uArray[nx][ny]=eqval; #print("uArray:"); print(uArray)
...
def coe(x,y,n):
    if n==1 or n==2:
        if (x<0.75 and x>0.25) and (y<0.75 and y>0.25):
            return 1
        else:
            return 9
    else:
