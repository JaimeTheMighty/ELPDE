...
#This version attempts to formulate the problem with a time component.
# It is not yet stochastic, but should not be hard to convert.
...
Notations used throughout the code:
    ...
    b1~b6 : set of boundary plains points
...
##For analyzing out of range points

#def useSolOut(model, Disc, deg, DiscT, shouldPlot, saveName, plotTitle):
#     ncoef = [[ m+1 for m in range(k+1)] for k in range(deg + 1)]
#     ####Discretize space, including the boundary points
#     X = list(np.linspace(0, 1, math.floor(1.0/Disc + 1)))
#     Y = list(np.linspace(0, 1, math.floor(1.0/Disc + 1)))
...
#     
#     ####Discretize time for plots, using DiscT
#     plotT = list(np.linspace(1, 2, math.floor(1.0/DiscT + 1)))
#     
#
#     xArray = np.linspace(0,1, math.floor(1.0/Disc + 1))
#     yArray = np.linspace(0,1, math.floor(1.0/Disc + 1))
...
useSol(model, Disc, deg, DiscT, True, "prob1_plot", r' ')